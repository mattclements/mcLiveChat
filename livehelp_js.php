...
if(!(empty($UNTRUSTED['winwidth']))){ $winwidth = $UNTRUSTED['winwidth']; }  
if(!(empty($UNTRUSTED['winheight']))){ $winheight = $UNTRUSTED['winheight']; }  
if(!(empty($UNTRUSTED['creditline']))){ $creditline = $UNTRUSTED['creditline']; } 
...
<?php } ?>
    ...
    <?php } else { ?>
 	    var newHTML =  '<a name="chatRef" href="javascript:openLiveHelp(<?php echo $UNTRUSTED['department']; ?>)" onclick="javascript:csTimeout_<?php echo $department; ?>=0;"><img name="csIcon" src="' + urltohelpimage_<?php echo $department; ?> + '" alt="Live Help" border="0"></a>';
...
?>
	...
	var urltohelpimage_<?php echo $department; ?> = WEBPATH + 'image.php?what=getstate&department=<?php echo $UNTRUSTED['department']; ?>&nowis=<?php echo date("YmdHis");  ?>&cslhVISITOR=<?php echo $identity['SESSIONID']; ?>' + 
					'&page=' + escape(locationvar) + 					
					'&referer=' + escape(var_referrer) + 					
					'&title=' + escape(var_title) + 
<?php if($UNTRUSTED['what'] =="hidden"){ echo "'&hide=Y + "; } ?>
  ...
  var urltocslhimage_<?php echo $department; ?> = WEBPATH + 'image.php?what=getcredit&department=<?php echo  $UNTRUSTED['department']; ?>&nowis=<?php echo date("YmdHis");  ?>&cslhVISITOR=<?php echo  $identity['SESSIONID']; ?>' + 
					'&xy=' + '<?php echo $xy;?>' + 
					'&page=' + escape(locationvar) + 
					'&referer=' + escape(var_referrer) +					
					'&title=' + escape(var_title) + 
